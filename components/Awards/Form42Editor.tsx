...
interface Form42EditorProps {
  onNext: (file: File) => void;
  onBack: () => void;
}

export default function Form42Editor({ onNext, onBack }: Form42EditorProps) {
  ...
  useEffect(() => {
    fetch('/IPA-Form4.2-2021Rev072021.docx')
  ...
  const handleNext = useCallback(async () => {
    ...
    try {
      ...
      const file = new File([blob], 'Form4.2.docx');
      onNext(file);
    } catch (error) {
  ...
  return (
    <div className="w-full">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-xl font-bold text-white">Form 4.2 - Journal Certification</h2>
